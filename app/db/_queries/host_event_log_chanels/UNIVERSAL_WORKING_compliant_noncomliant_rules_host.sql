-- For compliant rules use WHERE sr.id IN (SELECT sigma_id FROM compliant_rules)
-- For missing rules use WHERE sr.id NOT IN (SELECT sigma_id FROM compliant_rules)


WITH compliant_rules AS (
    SELECT DISTINCT sigma_id
    FROM host_sigma_compliance hsc
    WHERE hsc.host_id = 7  -- Replace with your host ID
    AND hsc.host_config_review_id = (
        SELECT latest_host_config_review_id 
        FROM host 
        WHERE id = 7  -- Replace with your host ID
    )
),
base_data AS (
    SELECT
